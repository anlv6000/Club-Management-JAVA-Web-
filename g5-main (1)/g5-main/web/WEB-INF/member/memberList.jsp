    ...
    <body>
        ...
        <div id="fh5co-page">
                ...
                <div class="container">
                    <div class="row">
                        <div style="display: flex; align-items: center; justify-content: space-between;">
                            <h1><span class="colored">Member</span></h1>
            ...
            <c:choose>
                <c:when test="${not empty sessionScope.messages}">
                    <div id="alertMessage" class="alert alert-${sessionScope.messageTypes}" role="alert">
                        ${sessionScope.messages}
                    </div>
                    <%-- Xóa messages khỏi session sau khi hiển thị --%>
                    <c:remove var="messages" scope="session"/>
                    <c:remove var="messageTypes" scope="session"/>
                </c:when>
                ...
                <div class="container">
                    <div class="row">
                        <div class="col-md-8 col-md-offset-2 text-center fh5co-heading animate-box" data-animate-effect="fadeIn">
                            <h2>People who share the same interests as you</h2>
                    ...
                    <div class="container">
                        ...
                        <ul class="nav nav-tabs" id="memberTabs" role="tablist">
                            <li class="nav-item">
                                <a class="nav-link ${param.tab ne 'pending' ? 'active' : ''}" id="official-tab" data-toggle="tab" href="#official" role="tab">Official Members</a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link ${param.tab eq 'pending' ? 'active' : ''}" id="pending-tab" data-toggle="tab" href="#pending" role="tab">Pending Members</a>
                            </li>
                        </ul>
                        ...
                        <div class="tab-content mt-3">
                            ...
                            <div class="tab-pane fade  " id="official" role="tabpanel">
                                <%-- BẮT ĐẦU NỘI DUNG CỦA `MemberList.jsp` --%>
                                    ...
                                    <div class="container-fluid fh5co-heading animate-box" data-animate-effect="fadeIn">
                                        <div class="d-flex justify-content-between align-items-center">
                                            <div class="left-section">
                                                ...
                                                <select id="clubDropdown" name="club" class="form-select" onchange="submitSearchForm()">
                                                    <c:forEach var="club" items="${clubs}">
                                                        <option value="${club.clubID}" ${param.club eq club.clubID ? 'selected' : ''}>${club.clubName}</option>
                                                    </c:forEach>
                                                </select>
                                            ...
                                            <div class="right-section">
                                                <input type="text" id="keyword" name="keyword" class="search-input" placeholder="Enter name">
                                ...
                                <table class="table table-bordered table-hover">
                                    ...
                                    <tbody>
                                        <c:if test="${empty officialMembers}">
                                            <tr>
                                                <td colspan="6" class="text-center">Danh sách thành viên rỗng!</td>
                                            </tr>
                                        </c:if>
                                        <c:forEach var="member" items="${officialMembers}">
                                            <tr>
                                                <td>
                                                    <img src="${member.imageURL}" alt="Avatar" class="avatar-img">
                                                </td>
                                                <td>
                                                    <a href="MemberOfficialDetail?userID=${member.userID}" class="btn-link">
                                                        ${member.userName}
                                                    </a>
                                                </td>
                                                <td>${member.phone}</td>
                                                <td>${member.role}</td>
                                                <td>
                                                    <span class="${member.status eq 'Active' ? 'status-active' : 'status-inactive'}">
                                                        ${member.status}
                                                    </span>
                                                </td>
                                                <td>
                                                    <a href="MemberOfficialDetail?userID=${member.userID}" class="btn btn-info btn-sm">View</a>
                                                </td>
                                            </tr>
                                        </c:forEach>
                                    </tbody>
                                ...
                                </div> <!-- Kết thúc Official Members Tab -->
                            ...
                            <div class="tab-pane fade " id="pending" role="tabpanel">
                                ...
                                <%-- BẮT ĐẦU NỘI DUNG CỦA `PendingMembers.jsp` --%>
                                <c:choose>
                                                        ...
                                                        <td>
                                                            <c:if test="${userRole eq 'ClubLeader'}">
                                                                <form action="ApproveMemberServlet" method="post" style="display:inline;">
                                                                    ...
                                                                    <input type="hidden" name="tab" value="pending"> <!-- Giữ lại tab -->
                                ...
                                <!-- Pagination cho Pending Members -->
                                <div class="row">
                                        ...
                                        <nav aria-label="Page navigation">
                                            <ul class="pagination">
                                                <c:if test="${pendingCurrentPage > 1}">
                                                    <li class="page-item">
                                                        <a class="page-link" href="ClubMember?pendingPage=${pendingCurrentPage - 1}">Previous</a>
                                                    </li>
                                                </c:if>
                                                <c:forEach var="i" begin="1" end="${pendingTotalPages}">
                                                    <li class="page-item ${i == pendingCurrentPage ? 'active' : ''}">
                                                        <a class="page-link" href="ClubMember?pendingPage=${i}">${i}</a>
                                                    </li>
                                                </c:forEach>
                                                <c:if test="${pendingCurrentPage < pendingTotalPages}">
                                                    <li class="page-item">
                                                        <a class="page-link" href="ClubMember?pendingPage=${pendingCurrentPage + 1}">Next</a>
                                                    </li>
                                                </c:if>
                                            </ul>
                                        </nav>
                                ...
                                <%-- KẾT THÚC NỘI DUNG CỦA `PendingMembers.jsp` --%>
                            </div>
                        </div>
                    </div>
        ...
        <script>
                                                    $(document).ready(function () {
                                                        ...
                                                        // Kích hoạt tab Official khi trang tải
