...
HttpSession session = request.getSession();
        ...
        if (dao.checkRegisExist(uid, cid)) {
        session.setAttribute("notification", "❌ Tài khoản đã tồn tại!");
        session.setAttribute("notificationType", "error");
    } else {
        dao.insertbyRegistered(uid, cid, note, phone);
        session.setAttribute("notification", "✅ Đơn đăng ký đang chờ xét duyệt!");
        session.setAttribute("notificationType", "success");
    }

    // Chuyển hướng về trang Categories.jsp
