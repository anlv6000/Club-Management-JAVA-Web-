...
public class requestPassword extends HttpServlet {
   private String emailErrorMessage = "";
    ...
    throws ServletException, IOException {
        ...
        if(!a.getPassword().contains("$")){
            request.setAttribute("mess", "you cannot change because you sign up by google");
                request.getRequestDispatcher("/WEB-INF/auth/requestPassword.jsp").forward(request, response);
            return;
        }
        resetService service = new resetService();
        String token = service.generateToken();
        String linkReset = "http://localhost:8080/club_manaaaa_end/resetPassword?token="+token;
    ...
    String password = "uzgv sary zaxw xkpa";  // Mật khẩu ứng dụng
    ...
    try {
        ...
        String content = "<h2>"+"From FPT ClubManagerment " + "</h2>"+
                "<h1>Hello " + name + "</h1>" +
