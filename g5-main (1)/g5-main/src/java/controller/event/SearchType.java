...
public class SearchType extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if("6".equals(RoleID)){
                 List<Event> eventList = eventService.getEventTypeByPage(page, pageSize,eventType);
                  int totalEvents = eventService.getTotalEvents();
            int totalPages = (int) Math.ceil((double) totalEvents / pageSize);

            // Lưu dữ liệu vào request để gửi đến JSP
            request.setAttribute("servlet", "searchType");
            request.setAttribute("queryParams", "&type=" + eventType);
          ...
          // Nếu cần hiển thị danh sách club
            ...
            List<Event> eventList = eventService.getEventByPageAndClubIDsSearh(page, pageSize, clubIDs,a,eventType);
            
            
            int totalEvents = eventService.getTotalEventsByNameAndClubIDs(a, clubIDs,eventType);
            int totalPages = (int) Math.ceil((double) totalEvents / pageSize);
